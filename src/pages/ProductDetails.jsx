...
import { faStar, faArrowLeft, faPlus, faMinus, faHeart as faHeartSolid, faChevronRight, faChevronLeft } from '@fortawesome/free-solid-svg-icons';
...
const ProductDetails = () => {
  ...
  const fetchBrandProducts = async (brandId, currentProductId) => {
    ...
    try {
      ...
      const response = await productService.getRelatedProducts(brandId, currentProductId);
  ...
  const moreFromBrand = [
    { id: 1, name: 'Product Name', price: '$250', rating: 5, image: '/products/product2.jpg' },
    { id: 2, name: 'Product Name', price: '$250', rating: 5, image: '/products/product3.jpg' },
  ];
  ...
  const reviews = [
    {
      ...
      name: 'Consumer name',
      rating: 4.0,
      comment: 'Lorem ipsum dolor sit amet, consectetur adipiscing elit. Vivamus felis justo, lacinia ac accumsan ut, efficitur elit.',
  ...
  if (!product) {
    return (
      <div className="min-h-screen pt-20 pb-10 px-4">
        <div className="container mx-auto max-w-4xl text-center">
          <h1 className="text-2xl font-bold text-light mb-4">Product Not Found</h1>
  ...
  return (
    <div className="min-h-screen bg-black text-white">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
              ...
              <div className="mb-6">
                <div className="flex justify-between items-start mb-2">
                  ...
                  <div className="flex items-center gap-1">
                    ...
                    <span className="text-sm text-gray-400">({product?.totalRatings || '0'})</span>
                  </div>
                </div>
              ...
              <div className="flex gap-4 mb-8">
                ...
                <button 
                  className={`flex-1 py-3 sm:py-4 rounded-lg font-medium text-sm sm:text-base transition-colors ${
                    isInWishlist ? 'bg-red-500 hover:bg-red-600' : 'bg-gray-700 hover:bg-gray-600'
                  }`}
              ...
              <div className="sm:col-span-2 lg:col-span-1">
                <div className="bg-secondary rounded-xl p-6">
                  <div className="flex justify-between items-center mb-4">
                    <h3 className="text-lg sm:text-xl font-semibold">About {product.brand.brand_name}</h3>
                  </div>
                  ...
                  <div className="grid grid-cols-3 gap-4 text-center">
                    <div>
                      <p className="text-xl sm:text-2xl font-semibold">{product.brand.successfulOrders || '30K+'}</p>
                      <p className="text-xs sm:text-sm text-gray-400">Orders</p>
                    </div>
                    <div>
                      <p className="text-xl sm:text-2xl font-semibold">{product.rating || '4.5'}</p>
                      <p className="text-xs sm:text-sm text-gray-400">Rating</p>
                    </div>
                    <div>
                      <p className="text-xl sm:text-2xl font-semibold">{product.brand.totalCustomers || '3K+'}</p>
                      <p className="text-xs sm:text-sm text-gray-400">Customers</p>
                    </div>
                  </div>
                </div>
              </div>
          ...
          <div className="mb-12">
            <div className="flex justify-between items-center mb-6">
              ...
              <button className="text-sm sm:text-base text-primary">See all reviews</button>
            </div>
                ...
                <div key={review.id} className="bg-secondary rounded-xl p-6">
                  <div className="flex items-center gap-3 mb-4">
                    ...
                    <div>
                      ...
                      <div className="flex items-center">
                        <span className="text-primary font-semibold mr-2">{review.rating}</span>
                        <FontAwesomeIcon icon={faStar} className="text-yellow-400 w-4 h-4" />
                      </div>
                    </div>
                  </div>
                  <p className="text-gray-400 text-sm sm:text-base">{review.comment}</p>
                </div>
              ))}
            </div>
          </div>
          ...
          {product?.brand && (
            <div className="mb-12">
              <div className="flex justify-between items-center mb-6">
                <h3 className="text-lg sm:text-xl font-semibold">More from {product.brand.brand_name}</h3>
                <Link to={`/brands/${product.brand.id}`} className="text-sm sm:text-base text-primary hover:underline">
                  See all
                </Link>
              </div>
      ...
      {/* Popup Message */}
      <div className={`fixed bottom-4 right-4 bg-gray-800 text-light px-6 py-3 rounded-md shadow-lg transition-opacity duration-300 ${
